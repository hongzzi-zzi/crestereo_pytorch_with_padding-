...
class InputPadder:
    ...
    def __init__(self, dims, mode='sintel', divis_by=8):
        ...
        # print(self._pad) # [14, 15, 0, 0]

    def pad(self, *inputs):
        assert all((x.ndim == 4) for x in inputs)
        return [F.pad(x, self._pad, mode='replicate') for x in inputs] 
        # Replication padding은 아래와 같이 가장 가까운 픽셀의 값을 복사해오는 방식이다.
        # 단, padding하는 위치가 멀어지면 그만큼 복사해오는 대상 픽셀의 위치도 멀어진다. 이렇게 하면 결과적으로 반전된 이미지가 새로 그려지게 된다.
